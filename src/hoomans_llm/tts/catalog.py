...
def _catalog_urlopen(request: Request, *args: Any, **kwargs: Any) -> Any:
    """Keep the historical ``hoomans_llm.tts.urlopen`` seam patchable."""

    from . import urlopen as configured_urlopen

    opener = configured_urlopen
    return opener(request, *args, **kwargs)


urlopen = _catalog_urlopen
...
class VoiceCatalog:
    ...
    def refresh_remote(self, *, force: bool = False) -> bool:
        ...
        try:
            request = Request(
                ...
                headers={"User-Agent": "HoomansLLM/0.1 Piper voice catalog"},
            )
    ...
    def download_sample(self, model_id: str) -> Path:
        ...
        descriptor, output_name = tempfile.mkstemp(prefix="hoomans-tts-preview-", suffix=".mp3")
        os.close(descriptor)
        ...
        try:
            request = Request(
                model.sample_url,
                headers={"User-Agent": "HoomansLLM/0.1 Piper voice preview"},
            )
    ...
    def _read_model(
        ...
        if isinstance(language, dict):
            ...
            language = str(language.get("family") or locale)
        else:
    ...
    def _download_verified(
        ...
        *,
        ...
        total = 0
        try:
            request = Request(url, headers={"User-Agent": "HoomansLLM/0.1 Piper voice installer"})
