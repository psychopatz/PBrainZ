...
class PiperProvider:
    ...
    def synthesize(self, text: str, voice_binding: VoiceBinding) -> SynthesizedAudio:
        ...
        preset = self.presets.all().get(voice_binding.slot)
        if not preset:
            raise TTSException(f"no Piper preset configured for {voice_binding.slot}")
        ...
        fd, output_name = tempfile.mkstemp(prefix="hoomans-tts-", suffix=".wav")
        os.close(fd)
